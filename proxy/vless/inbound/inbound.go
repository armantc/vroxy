...
func (h *Handler) Process(ctx context.Context, network net.Network, connection stat.Connection, dispatcher routing.Dispatcher) error {
	iConn := connection
	var _statConn *stat.CounterConnection
	...
	if _statConn != nil {
		authSuccess := _statConn.AuthenticateConnection(account.ID.String()) //authenticate and create connection

		if !authSuccess {
			return errors.New("Failed to validate auth , uuid : " + account.ID.String()).Base(err).AtInfo()
		}
	}
